...
This document is the detailed database design input for a future Constructive DB pull request. It derives its behavioral requirements from:

- [`docs/plan/oauth-sso-platform-integration.md`](../plan/oauth-sso-platform-integration.md); and
- [`docs/spec/oauth-sso-platform-integration.md`](./oauth-sso-platform-integration.md).
...
13. External identity association remains owned by `connected_accounts`, keyed by Provider `service` plus stable external `identifier`, never email or an authorization code.
14. `sign_in_identity` and `sign_up_identity` remain unchanged general Tenant **external-identity** authentication primitives. SSO-specific validation and orchestration belong in wrappers or coordinating functions.
15. Every successful authentication method converges on the same handoff and Site-local credential path.
16. Caught database errors are either mapped to a registered stable domain error or rethrown with their cause preserved. Logging or fallback behavior does not replace failure semantics.
...
| Browser binding | Prevents a transaction created in one browser context from being spent by another | **Confirmed behavior**; use the authentication center's first-party session/request context as the authority; storage details are a DB PR decision |
...
- Site, callback, Tenant, group, browser, and `returnTo` cannot be replaced by later browser input (**Confirmed**).
...
| Create Provider OAuth request | Active transaction ID, selected enabled Provider, random state, PKCE verifier, optional nonce, and exact Provider redirect URI | Private OAuth request ID; opaque OAuth state is emitted to the service | Validate transaction/Provider/Tenant before insert, persist the unified-transaction link before redirect, and enforce the confirmed ten-minute expiry. The DB PR assigns cryptographically secure generation to the correct service/DB owner; only the challenge crosses the browser boundary. |
...
| Create SSO handoff | Authenticated transaction and newly generated plaintext code/hash pair | Plaintext handoff code exactly once plus expiry | Lock transaction; revalidate its authoritative Tenant/Site/callback/group; insert only the secure hash and confirmed minimal fields; enforce one-minute expiry. The DB PR assigns secure plaintext generation to the correct service/DB owner. |
| Redeem SSO handoff | Plaintext code plus authenticated target Site/runtime and request context | Distinct Site-local credential result and verified Site-internal `returnTo` | Hash and lock the candidate row; follow its transaction; reuse the existing platform Site/runtime authentication capability wherever available; require the authenticated Site/request context to resolve to the same Tenant; validate unexpired/unused and all other boundaries; issue Site-local credential; stamp `consumed_at` only after issuance succeeds. If the current capability cannot express the Site/runtime identity, the DB PR must identify the correct integration point from live evidence and must not invent a new SSO credential system. The whole success path commits or rolls back together. |
...
| Provider authorization initiation | Browser HTTP operation coordinated by Constructive | Active transaction, validated Provider, and current route/context | Persists the linked OAuth request before browser redirect. This remains HTTP because redirect semantics are not replaceable by GraphQL. |
...
The Site callback itself remains an exact registered browser `POST` destination. Constructive cannot set a Cookie for the Site's parent domain; after server-side redemption, the Site response owns its first-party Cookie and frontend Bearer-storage behavior.
...
| Browser callback delivery | No private DB read in browser | No DB write | Dashboard posts plaintext code to the exact callback; code is never in the URL. |
...
- Unified transactions, OAuth authorization requests, handoffs, browser bindings, and identity outcomes are server-only private state.
...
- No migration adds legacy multi-version lookup, old-schema fallback, parent-domain shared-cookie behavior, or URL token transport.
...
- opaque identifier/state/hash-at-rest representations, cryptographic primitives approved by the repository, and whether secure random material is generated by Constructive or PostgreSQL;
