 ...
 * 
 * Follows the spec from:
 *   Prompts-collection/constructive/graphql-server/perf/find-graphile-cache-potential-leak-test-spec.md
 * 
 * Sends REAL GraphQL HTTP requests through the Express server,
 * exercising the full PostGraphile/Grafast pipeline.
 * 
 * Usage:
 *   MODE=old|new K=20 DURATION=300 WORKERS=8 npx ts-node e2e-benchmark.ts
 */
...
async function getHeapUsedMB(): Promise<number> {
  ...
  try {
    const res = await new Promise<string>((resolve, reject) => {
      http.get(`http://${SERVER_HOST}:${SERVER_PORT}/debug/memory`, (res) => {
        let data = '';
        res.on('data', (chunk) => (data += chunk));
    ...
    // fallback: use process.memoryUsage via a GraphQL introspection as proxy
  }

  // Fallback: measure from client side (less accurate but still useful)
  // Force GC via the server if possible, otherwise just report Node heap
...
function buildTenantProfiles(k: number): TenantProfile[] {
  ...
  for (let i = 0; i < k; i++) {
    ...
    // Use unique database IDs to create distinct cache keys per tenant
    // All point to the same physical database (postgres) via X-Schemata mode
...
async function pressureWorker(
  ...
  while (Date.now() < endTime) {
    // Pick random tenant and operation
...
async function runBenchmark(): Promise<BenchmarkResult> {
  ...
  // Phase 5: Post-pressure heap snapshot
  ...
  console.log(`  Cold Start (first/last): ${result.coldStartMs[0]}ms / ${result.coldStartMs[result.coldStartMs.length - 1]}ms`);
  console.log('─'.repeat(70));

  // Write result to JSON file
  const outFile = `/tmp/e2e-benchmark-${MODE}-k${K}.json`;
