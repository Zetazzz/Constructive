...
import { getGraphQLEnvVars } from '@constructive-io/graphql-env';
...
export function resolveEmbedder(store?: {
  ...
  const envEmbedder = getGraphQLEnvVars().llm?.embedder;
  if (envEmbedder?.provider) {
    return buildEmbedder({
      provider: envEmbedder.provider,
      model: envEmbedder.model,
      baseUrl: envEmbedder.baseUrl,
    });
  }
