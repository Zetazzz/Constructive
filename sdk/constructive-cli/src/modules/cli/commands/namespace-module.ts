/**
 * CLI commands for NamespaceModule
...
async function handleList(argv: Partial<Record<string, unknown>>, _prompter: Inquirerer) {
  try {
    const defaultSelect = {
      ...
      namespaceEventsTableId: true,
      namespacesTableName: true,
      namespaceEventsTableName: true,
      apiName: true,
      privateApiName: true,
      scope: true,
      prefix: true,
    ...
    const result = await client.namespaceModule.findMany(findManyArgs).execute();
...
async function handleFindFirst(argv: Partial<Record<string, unknown>>, _prompter: Inquirerer) {
  try {
    ...
    >(argv, defaultSelect);
    const client = getClient();
    const result = await client.namespaceModule.findFirst(findFirstArgs).execute();
    console.log(JSON.stringify(result, null, 2));
  } catch (error) {
...
async function handleCreate(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    const rawAnswers = await prompter.prompt(argv, [
      ...
      {
        type: 'text',
        name: 'namespaceEventsTableId',
        message: 'namespaceEventsTableId',
        required: false,
        skipPrompt: true,
      },
      {
      ...
      {
        type: 'text',
        name: 'namespaceEventsTableName',
        message: 'namespaceEventsTableName',
        required: false,
        skipPrompt: true,
      },
      {
    ...
    const cleanedData = stripUndefined(
      answers,
      fieldSchema
    ) as CreateNamespaceModuleInput['namespaceModule'];
    const client = getClient();
    const result = await client.namespaceModule
      .create({
        data: {
          ...
          namespaceEventsTableId: cleanedData.namespaceEventsTableId,
          namespacesTableName: cleanedData.namespacesTableName,
          namespaceEventsTableName: cleanedData.namespaceEventsTableName,
